...
def ad_knot_mu_sensitivities(L: int) -> np.ndarray:
    if L % 2 != 1 or L < 3:
        raise ValueError("L must be odd and >= 3.")
    k = (L - 1) // 2
    left = np.array([j / (k + 1) for j in range(1, k + 1)], dtype=float)
    center = np.array([1.0], dtype=float)
    right = np.array([1.0 - j / (k + 1) for j in range(1, k + 1)], dtype=float)
    return np.concatenate([left, center, right]).astype(float)
...
def band_builder_paper(samples: np.ndarray, a: float, b: float, mode: str, eps: float):
    ...
    if mode.startswith("Mean-AD"):
        ...
        sens = ad_knot_mu_sensitivities(L)

        da_hat = float(np.mean(np.abs(samples - a)))
        db_hat = float(np.mean(np.abs(samples - b)))

        T_cs = [(float(c - s * eps), float(c + s * eps)) for c, s in zip(cs, sens)]
        D_cs = [(float(ad - eps), float(ad + eps)) for ad in ad_hats]
        ...
        out.update({
            ...
            "T_cs": T_cs,
            "D_cs": D_cs,
            "D_a": (da_hat - eps, da_hat + eps),
            "D_b": (db_hat - eps, db_hat + eps),
            "D_mu": (float(ad_hats[mid] - eps), float(ad_hats[mid] + eps)),
    ...
    for C1 in C1_candidates:
        ...
        for i in range(k):
            ...
            if mode == "Mean-MAD":
                ...
                L = int(mode.split("AD")[1])
                ad_uppers = np.array([float(Dc[1]) for Dc in dd["D_cs"]], dtype=float)
                for qq in q_grid:
...
def make_N_grid():
    out = []
    out += list(range(2, 12, 2))
    out += list(range(15, 55, 5))
    out += list(range(60, 110, 10))
    out += list(range(120, 201, 20))
    return out
...
with st.sidebar:
    ...
    if do_sweep:
        ...
        needs_ad5_cv = any("Mean-AD5 (banded: k-fold cv)" == s for s in sweep_choices)
        needs_ad7_cv = any("Mean-AD7 (banded: k-fold cv)" == s for s in sweep_choices)
        needs_ad9_cv = any("Mean-AD9 (banded: k-fold cv)" == s for s in sweep_choices)
        ...
        sweep_ad5_cv_use = None
        sweep_ad5_C1_candidates = None
        sweep_ad5_C1_manual = None

        sweep_ad7_cv_use = None
        sweep_ad7_C1_candidates = None
        sweep_ad7_C1_manual = None

        sweep_ad9_cv_use = None
        sweep_ad9_C1_candidates = None
        sweep_ad9_C1_manual = None

        sweep_mad_alpha = None
        sweep_ad3_alpha = None

        if needs_mad_cv or needs_ad3_cv or needs_ad5_cv or needs_ad7_cv or needs_ad9_cv or needs_mad_h or needs_ad3_h:
            st.divider()
        ...
        if needs_ad5_cv:
            st.markdown("**Mean-AD5 (banded: k-fold cv)**")
            sweep_ad5_C1_candidates_default = [5.0, 1.0, 0.5, 0.1, 0.05, 0.01]
            sweep_ad5_C1_candidates_text = st.text_input(
                "Mean-AD5 CV: C1 candidates (comma-separated)",
                value=", ".join(map(str, sweep_ad5_C1_candidates_default)),
                key="sweep_ad5_c1_candidates_text",
            )
            sweep_ad5_C1_candidates = parse_candidate_text(sweep_ad5_C1_candidates_text, sweep_ad5_C1_candidates_default)
            sweep_ad5_cv_use = st.checkbox("Mean-AD5 CV: select C1 via k-fold CV", value=True, key="sweep_ad5_usecv")
            if not sweep_ad5_cv_use:
                sweep_ad5_C1_manual = float(st.select_slider("Mean-AD5 CV: pick C1 manually", options=sweep_ad5_C1_candidates, value=manual_candidate_default(sweep_ad5_C1_candidates), key="sweep_ad5_c1"))

        if needs_ad7_cv:
            st.markdown("**Mean-AD7 (banded: k-fold cv)**")
            sweep_ad7_C1_candidates_default = [5.0, 1.0, 0.5, 0.1, 0.05, 0.01]
            sweep_ad7_C1_candidates_text = st.text_input(
                "Mean-AD7 CV: C1 candidates (comma-separated)",
                value=", ".join(map(str, sweep_ad7_C1_candidates_default)),
                key="sweep_ad7_c1_candidates_text",
            )
            sweep_ad7_C1_candidates = parse_candidate_text(sweep_ad7_C1_candidates_text, sweep_ad7_C1_candidates_default)
            sweep_ad7_cv_use = st.checkbox("Mean-AD7 CV: select C1 via k-fold CV", value=True, key="sweep_ad7_usecv")
            if not sweep_ad7_cv_use:
                sweep_ad7_C1_manual = float(st.select_slider("Mean-AD7 CV: pick C1 manually", options=sweep_ad7_C1_candidates, value=manual_candidate_default(sweep_ad7_C1_candidates), key="sweep_ad7_c1"))

        if needs_ad9_cv:
            st.markdown("**Mean-AD9 (banded: k-fold cv)**")
            sweep_ad9_C1_candidates_default = [5.0, 1.0, 0.5, 0.1, 0.05, 0.01]
            sweep_ad9_C1_candidates_text = st.text_input(
                "Mean-AD9 CV: C1 candidates (comma-separated)",
                value=", ".join(map(str, sweep_ad9_C1_candidates_default)),
                key="sweep_ad9_c1_candidates_text",
            )
            sweep_ad9_C1_candidates = parse_candidate_text(sweep_ad9_C1_candidates_text, sweep_ad9_C1_candidates_default)
            sweep_ad9_cv_use = st.checkbox("Mean-AD9 CV: select C1 via k-fold CV", value=True, key="sweep_ad9_usecv")
            if not sweep_ad9_cv_use:
                sweep_ad9_C1_manual = float(st.select_slider("Mean-AD9 CV: pick C1 manually", options=sweep_ad9_C1_candidates, value=manual_candidate_default(sweep_ad9_C1_candidates), key="sweep_ad9_c1"))
        ...
        for lab in sweep_choices:
            ...
            if bm == "k-fold cv":
                ...
                if md == "Mean-MAD":
                    ...
                    spec["cv_use"] = sweep_ad5_cv_use if sweep_ad5_cv_use is not None else True
                    spec["C1_candidates"] = sweep_ad5_C1_candidates if sweep_ad5_C1_candidates is not None else [5.0, 1.0, 0.5, 0.1, 0.05, 0.01]
                    spec["C1_manual"] = sweep_ad5_C1_manual
                elif md == "Mean-AD7":
                    spec["cv_use"] = sweep_ad7_cv_use if sweep_ad7_cv_use is not None else True
                    spec["C1_candidates"] = sweep_ad7_C1_candidates if sweep_ad7_C1_candidates is not None else [5.0, 1.0, 0.5, 0.1, 0.05, 0.01]
                    spec["C1_manual"] = sweep_ad7_C1_manual
                elif md == "Mean-AD9":
                    spec["cv_use"] = sweep_ad9_cv_use if sweep_ad9_cv_use is not None else True
                    spec["C1_candidates"] = sweep_ad9_C1_candidates if sweep_ad9_C1_candidates is not None else [5.0, 1.0, 0.5, 0.1, 0.05, 0.01]
                    spec["C1_manual"] = sweep_ad9_C1_manual
